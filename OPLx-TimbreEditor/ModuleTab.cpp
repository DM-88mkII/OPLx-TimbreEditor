	...
	switch (pMsg->message){
		case WM_KEYDOWN:{
			...
			if (bControl){
				...
				switch (pMsg->wParam){
					case VK_HOME:{		m_Octave += (m_Octave < 9)? 1: 0;	Log(_T("o{}"), m_Octave);	return TRUE;	}
					case VK_END:{		m_Octave -= (m_Octave > 0)? 1: 0;	Log(_T("o{}"), m_Octave);	return TRUE;	}
...
void CModuleTab::OnBnClickedModuleLoadButton()
{
	...
	CFileDialog FileDialog(true, _T("oplx"), _T("*.oplx"), OFN_FILEMUSTEXIST | OFN_EXTENSIONDIFFERENT);
...
void CModuleTab::OnBnClickedModuleSaveButton()
{
	...
	CFileDialog FileDialog(false, _T("oplx"), _T("*.oplx"), OFN_OVERWRITEPROMPT | OFN_EXTENSIONDIFFERENT);
	if (FileDialog.DoModal() == IDOK){
		SetTabName(FileDialog.GetFileTitle());
...
void CModuleTab::Play(bool bShift, int Note, CString Key)
{
	...
	Log(_T("o{}{}({})"), (Note / 12), (LPCTSTR)Key, Note);
